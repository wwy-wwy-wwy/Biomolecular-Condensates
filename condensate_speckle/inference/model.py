...
def set_model(data,quantization):
    ...
    with ar1_model:
        ...
        camera_noise_std_center = observed_mean/12
        #camera_noise_std = pm.Uniform("noise_std", lower=0, upper=quantization)
        camera_noise_std = pm.Normal("noise_std", mu=camera_noise_std_center, sigma=5)
...
def plot_pair(trace, n_time_scale = 1, var_names = ['decay_time','precision','noise_std']):
    '''
    This function plots the posterior of sampling from the single or the multiple time scale model
